...
async def mark_list_create(markListBody: MarkListCreateBody, request: Request):
    ...
    MarkLists.create(userUUID, markListBody.title, markListBody.shared, markListBody.description)
...
async def mark_list_remove(markListBody: MarkListRemoveBody, request: Request):
    userUUID = userUUID_get(request)
    user_verify_by_uuid(userUUID, True)
    info = MarkLists.info(markListBody.markListUUID)
    mark_list_permission_verify(info, userUUID)
    MarkLists.remove(markListBody.markListUUID)
    return return_handler()


@router.get("/mark_list/info")
async def mark_list_info(markListUUID: str):
    info = MarkLists.info(markListUUID)
...
async def mark_list_get(request: Request):
    userUUID = userUUID_get(request)
    user_verify_by_uuid(userUUID, True)
    mark_lists = MarkLists.get_mark_lists(userUUID)
    return return_handler(
...
async def mark_list_get_marks(markListUUID: str, request: Request):
    ...
    if not info[4]:
        mark_list_permission_verify(info, userUUID)
    marks = MarkLists.get_marks(markListUUID)
...
async def mark_list_add_mark(markBody: MarkListAddMarkBody, request: Request):
    userUUID = userUUID_get(request)
    user_verify_by_uuid(userUUID, True)
    info = MarkLists.info(markBody.markListUUID)
    mark_list_permission_verify(info, userUUID)
    MarkLists.add_mark(markBody.markListUUID, markBody.markUUID)
    return return_handler()
...
async def mark_list_remove_mark(markBody: MarkListRemoveMarkBody, request: Request):
    ...
    info = MarkLists.info(markBody.markListUUID)
    mark_list_permission_verify(info, userUUID)
    MarkLists.remove_mark(markBody.markListUUID, markBody.markUUID)
    return return_handler()
...
async def mark_list_get_marks_by_uid(markBody: MarkListGetByUidBody, request: Request):
    ...
    if not info[4]:
        mark_list_permission_verify(info, userUUID)
    marks = MarkLists.get_by_uid(markBody.markListUUID, markBody.uid)
    return return_handler([{"markUUID": i[0], "uid": i[2], "reason": i[3], "evidence": i[4]} for i in marks])
